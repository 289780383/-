...
$(function() {
    ...
    /**订单排序*/
    $(".order_sort li").on("click", order_sort);
    /**订单查找*/
    ...
    /**日期插件*/
    laydate({
        elem: '#form_date', //目标元素。由于laydate.js封装了一个轻量级的选择器引擎，因此elem还允许你传入class、tag但必须按照这种方式 '#id .class'
    ...
    $("#start_poi, #end_poi").on('keyup', function() {
        input_auto("", $(this));
    });
    $("#start_poi2, #end_poi2").on('keyup', function() {
...
function order_sort() {
    $(this).siblings().removeClass("sel_sort");
...
function order_nav() {
    ...
    $(".order_sort li").eq(0).trigger("click");
...
function order_sure() {
    ...
    $(".sure_table .order_sort li").eq(0).trigger("click");
}
/**设置列表高度*/
...
function set_height() {
    ...
    $(".order_bot").css("height", (Hight - 300) + "px");
}
/**下拉列表select*/
...
function order_pai(data, page, click_nav) {
    ...
    for (var i = (page * 10) - 10; i < page * 10; i++) {
        ...
        if (data[i].pai_over) {
            ...
            str += '<div class="submit_warp submit_over"><p>车号：</p><input type="text" readonly class="driver_id" value="' + data[i].car_id + '"><p>司机：</p><input type="text" readonly class="driver_name" value="' + data[i].driver_name + '"><p>手机号：</p><input type="tel" readonly class="driver_tel" value="' + data[i].driver_tel + '"><div class="dispatch_cancel submit_btn">取消派单</div></div></li>';
        } else {
            str += '<div class="dispatch_sure dispatch_btn">派单</div>';
            str += '<div class="submit_warp"><p>车号：</p><input type="text" class="driver_id" placeholder="输入车号"><p>司机：</p><input type="text" class="driver_name" placeholder="输入姓名"><p>手机号：</p><input type="tel" class="driver_tel" placeholder="输入手机号"><div class="dispatch_submit submit_btn">提交</div></div></li>';
        }
