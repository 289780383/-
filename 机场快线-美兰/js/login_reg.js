...
/**导航切换*/
function nav_fn(){
	$(".login_nav li").removeClass("sel_li");
...
function tel_or(val){
	if(val.length==11){
		if (!/^(13[0-9]|14[579]|15[012356789]|17[0135678]|18[0-9])\d{4}\d{4}$/.test(val)){
			msg_show1("手机号格式有误");
			$(".register_warp,.reg_over,.login_nav").hide();
			$(".yzm_btn").text("重新发送");
			$(".yzm_btn").on("click",yzm_btn);
			clearInterval(time_yzm);
			return;
            ...
            success: function (data) {
            	...
            	if(data.returnCode==1){
            		if(data.returnMsg.memberState!=1){
            			...
            			$(".reg_over,.login_nav").show();
            			$(".login_nav li").eq(0).trigger("click");
            		}
            	}else{
            		msg_show1("请输入正确手机号");
            	}
